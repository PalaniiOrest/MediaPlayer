            ...
            if (avcodec_send_packet(m_codecCtx, &packet) == 0) {
                if (avcodec_receive_frame(m_codecCtx, m_hwFrame) == 0) {
                    ...
                    if (m_hwFrame->format == AV_PIX_FMT_D3D11) {
                        ...
                        av_image_alloc(m_bgraFrame->data, m_bgraFrame->linesize, m_bgraFrame->width, m_bgraFrame->height, AV_PIX_FMT_BGRA, 32);

                        sws_scale(sws_ctx, m_swFrame->data, m_swFrame->linesize, 0, m_swFrame->height, m_bgraFrame->data, m_bgraFrame->linesize);
