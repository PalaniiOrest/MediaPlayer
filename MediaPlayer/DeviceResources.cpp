...
void DeviceResources::SetSwapChainPanel(const winrt::Microsoft::UI::Xaml::Controls::SwapChainPanel& panel)
{
	...
	m_logicalSize = winrt::Windows::Foundation::Size(
...
void DeviceResources::ValidateDevice()
{
	...
	com_ptr<IDXGIDevice3> dxgiDevice = m_d3dDevice.as<IDXGIDevice3>();

	com_ptr<IDXGIAdapter> deviceAdapter;
	check_hresult(dxgiDevice->GetAdapter(deviceAdapter.put()));

	com_ptr<IDXGIFactory2> deviceFactory;
	check_hresult(deviceAdapter->GetParent(IID_PPV_ARGS(&deviceFactory)));

	com_ptr<IDXGIAdapter1> previousDefaultAdapter;
	check_hresult(deviceFactory->EnumAdapters1(0, previousDefaultAdapter.put()));

	DXGI_ADAPTER_DESC1 previousDesc;
	check_hresult(previousDefaultAdapter->GetDesc1(&previousDesc));

	// Next, get the information for the current default adapter.

	com_ptr<IDXGIFactory4> currentFactory;
	check_hresult(CreateDXGIFactory1(IID_PPV_ARGS(&currentFactory)));

	com_ptr<IDXGIAdapter1> currentDefaultAdapter;
	check_hresult(currentFactory->EnumAdapters1(0, currentDefaultAdapter.put()));

	DXGI_ADAPTER_DESC1 currentDesc;
	check_hresult(currentDefaultAdapter->GetDesc1(&currentDesc));
...
void DeviceResources::Trim() const
{
	com_ptr<IDXGIDevice3> dxgiDevice = m_d3dDevice.as<IDXGIDevice3>();
...
void DeviceResources::CreateDeviceIndependentResources()
{
	...
	check_hresult(
		DWriteCreateFactory(
			...
			m_dwriteFactory.as<::IUnknown>().put()
		)
	);
...
void DeviceResources::CreateWindowSizeDependentResources()
{
	...
	default:
		throw winrt::hresult_error(E_FAIL, L"Failure occurred");
	}
	...
	com_ptr<IDXGISwapChain2> spSwapChain2 = m_swapChain.as<IDXGISwapChain2>();
