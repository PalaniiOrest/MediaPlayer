...
MFVideoDecoder::MFVideoDecoder(const std::shared_ptr<DeviceResources>& deviceResources)
...
void MFVideoDecoder::decodeFrame(VideoFrame& frame)
{
...
uint32_t MFVideoDecoder::getVideoWidth() const
{
	return m_videoWidth;
}

uint32_t MFVideoDecoder::getVideoHeight() const
{
	return m_videoHeight;
}

uint64_t MFVideoDecoder::getCurrentPosition() const
{
	return m_currentPosition;
}

uint64_t MFVideoDecoder::getMediaDuration() const
{
	return m_mediaDuration;
}

uint64_t MFVideoDecoder::getFrameDuration() const
{
	return m_frameDuration;
}

bool MFVideoDecoder::getIsEndOfStream() const
{
	return m_isEndOfStream;
}

void MFVideoDecoder::initializeD3DManager()
{
	UINT resetToken = 0;
...
void MFVideoDecoder::calculateMediaDuration()
{
...
void MFVideoDecoder::configureVideoStream()
{
...
bool MFVideoDecoder::isHardwareDecoderActive()
{
	if (!m_sourceReader)
	{
		return false;
